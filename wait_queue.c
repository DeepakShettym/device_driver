...
wait_queue_head_t wait_queue_dms;
...
static int wait_function(void* data)
{
        while (1) {
                ...
                wait_event_interruptible(wait_queue_dms, wait_queue_flag != 0);
...
static ssize_t dms_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
        ...
        wait_queue_flag = 1;
        wake_up_interruptible(&wait_queue_dms);
        return 0;
}
...
static int __init dms_driver_init(void)
{
        ...
        init_waitqueue_head(&wait_queue_dms);
