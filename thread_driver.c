...
static struct task_struct *dms_thread;
...
{
    int i=0;
    while(!kthread_should_stop()) {   // break's when kthread_stop is called 
        pr_info("In DeepakM Thread Function %d\n", i++);
        msleep(1000);
    }
    return 0;
}
...
static int __init dms_driver_init(void)
{
        ...
        // create thread
        dms_thread = kthread_create(thread_function,NULL,"dms Thread");
        if(dms_thread) {
            wake_up_process(dms_thread);  //wake up the created thread from sleep initially
        } else {
        ...
        dms_thread = kthread_run(thread_function,NULL,"dms Thread");
        if(dms_thread) {
...
static void __exit dms_driver_exit(void)
{
        kthread_stop(dms_thread); // stop the thread
